...
public class OrderDao {
	...
	public OrderVo findbykey(Connection _conn, OrderVo vo) {
		...
		try {
			...
			ps = _conn.prepareStatement(sbSQL.toString());
			int nIndex = 1;
	...
	public List findbyAll(Connection _conn, String strSql) {
		...
		// 定义数据库链接
		Connection conn = null;
		// 数据中查询结果返回的一种对象,可以说结果集是一个存储查询结果的对象
		ResultSet rs = null;
		try {
			conn = _conn;
			sbSQL = new StringBuffer();
	...
	public void insert(Connection _conn, OrderVo vo) {
		...
		try {
			...
			/*
			 * if(vo.get()==0){ String _sql = "select max() from Order"; Statement
			 * _st=_conn.createStatement(); ResultSet rs = _st.executeQuery(_sql);
			 * while(rs.next()){ vo.set(rs.getInt(1)+1); } }
			 */
			// 定义参数递增变量
			int nIndex = 1;
