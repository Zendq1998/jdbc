...
<body>
	<h1>订单列表</h1>
	<div>
		<a href="<%=path%>/servlets/GoToAddOrder.html">添加订单</a>
	</div>
	<table>
		<tr>
			<th>消费人</th>
		...
		<tr>
			...
			<td>
				<%
					if (vo.getState() == 0) {
				%><a
				href="<%=path%>/servlets/DealOrder.html?state=1&id=<%=vo.getId()%>">支付</a>&nbsp;<a
				href="<%=path%>/servlets/GoToUpdateOrder.html?state=1&id=<%=vo.getId()%>">修改</a>&nbsp;<a
				href="<%=path%>/servlets/DeleteOrder.html?id=<%=vo.getId()%>">删除</a>
				<%
					}
				%> &nbsp;
			</td>
